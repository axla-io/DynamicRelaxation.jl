...
function constrained_dofs(u_len, simulation::StructuralSimulation{Vector{Node6DOF}})
...
function constrained_dofs(u_len, simulation::StructuralSimulation{Vector{Node3DOF}})
	...
	for (i, body) in enumerate(simulation.system.bodies) # Loop through bodies
		if body.constrained == true
			for (j, cons) in enumerate(body.constraints) # Loop through constraints if the body is constrained
				if cons #if constrained
					dofs[ctr] = (i - 1) * 3 + j # Add position dof to constrained dof list
					ctr += 1
					dofs[ctr] = u_len + (i - 1) * 3 + j # Add velocity dof to constrained dof list
					ctr += 1
				end
			end
		end
...
function get_jac_prototype(system, u_len, v_len, simulation::StructuralSimulation{Vector{Node6DOF}}, apply_constraints)
	dim = u_len + v_len
	jac_p = zeros(dim, dim)
...
function get_jac_prototype(system, u_len, v_len, simulation::StructuralSimulation{Vector{Node3DOF}}, apply_constraints)
	dim = u_len + v_len
	jac_p = zeros(dim, dim)
	n_pt = length(system.bodies)
	...
	for edge in edges(system.graph)
		...
		# Contribution to dv
		# b to a
		jac_p[va_0+1:va_0+3, xa_0+1:xa_0+3] .= 1.0
		jac_p[va_0+1:va_0+3, xb_0+1:xb_0+3] .= 1.0

		# a to b
		jac_p[vb_0+1:vb_0+3, xb_0+1:xb_0+3] .= 1.0
		jac_p[vb_0+1:vb_0+3, xa_0+1:xa_0+3] .= 1.0
	end
