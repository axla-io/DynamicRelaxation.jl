abstract type AbstractGraphSystem end
struct StructuralGraphSystem{bType <: AbstractNode, gType <: AbstractGraph, EType <: AbstractElement} <:
       AbstractGraphSystem
    # nbody
    bodies::Vector{bType}
    ...
    # custom
    elem_props::Vector{EType} # Also check how to properly set element properties
    edgemap::Dict{Tuple{Int, Int}, Int}
end

function StructuralGraphSystem{bT}(nodes, sg::gT, elem_props::Vector{eT}, edgemap) where {bT, gT, eT}
    return StructuralGraphSystem{bT, gT, eT}(nodes, sg, elem_props, edgemap)
end

function edge_index(e::Union{Tuple{Int, Int}, Tuple{UInt8, UInt8},Tuple{Int, UInt16},Tuple{Int, UInt8},},
